...
if(infilex=="gz") {
  ...
  d1<-read.table(gzfile(args[1]))
}
#d1<-d
#d1<-read.table(gzfile('tempall/data/annotbest.txt.gz'))
...
plot(1,type="n",xlim=c(0,longest),ylim=c(0,biggest*1.1),ylab="Count reads per aligned length",bty="n",xlab="Reference transcript length (bp)",xaxt='n',cex.axis=1.2,cex.lab=1.2)
...
recwid = 1
endspace = 4
plot(1,type="n",xlim=c(0,maxexon+endspace),ylim=c(0,logtrans(biggest*2)),ylab="Count reads",bty="n",xlab="Exon count (reference transcript)",xaxt='n',cex.axis=1.2,cex.lab=1.2,yaxt='n')
